...
router.get('/getByRegion', async (req, res) => {
  try {    
    ...
    const [results] = await db.query(query, [regione, regione,regione, regione, regione !== "VALLE D'AOSTA" ? regione.replace(/ /, "-") : "VALLE D'AOSTA"]);
...
router.get('/getSudNord', async (req, res) => {
  try {    
